...
export default function HalamanKameraWajah() {
  ...
  useEffect(() => { if (appState !== 'CAMERA' && appState !== 'CONFIRM') return; let currentStream: MediaStream | null = null; const startCamera = async () => { try { const mediaStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user', width: { ideal: 1920 }, height: { ideal: 1080 } } }); currentStream = mediaStream; if (videoRef.current) { videoRef.current.srcObject = mediaStream; } } catch { setError('Kamera tidak dapat diakses.'); } }; startCamera(); return () => { if (currentStream) currentStream.getTracks().forEach((track) => track.stop()); }; }, [appState]);
  useEffect(() => { if (appState === 'ANALYZING') { setLoadingStep(0); setProgress(0); const stepCount = LOADING_STEPS.length; const totalDuration = 4000 + stepCount * 1200; const stepDuration = 1200; const stepTimer = setInterval(() => { setLoadingStep((prev) => { if (prev < stepCount - 1) { return prev + 1; } else { clearInterval(stepTimer); return prev; } }); }, stepDuration); const progressTimer = setInterval(() => { setProgress((prev) => { if (prev >= 99) { clearInterval(progressTimer); return 99; } return prev + 1; }); }, Math.max(20, totalDuration / 100)); const finishTimer = setTimeout(() => { setProgress(100); setAppState('RESULTS'); }, totalDuration); return () => { clearInterval(stepTimer); clearInterval(progressTimer); clearTimeout(finishTimer); }; } }, [appState]);
  useEffect(() => { if (appState === 'RESULTS') { const animationTimer = setInterval(() => { setCompletedAnalyses(prev => { if (prev >= totalAnalyses) { clearInterval(animationTimer); return prev; } return prev + 1; }); }, 700); return () => clearInterval(animationTimer); } }, [appState]);
  useEffect(() => { if (completedAnalyses >= totalAnalyses) { const redirectTimer = setTimeout(() => { router.push('/ai-overview'); }, 1000); return () => clearTimeout(redirectTimer); } }, [completedAnalyses, router, totalAnalyses]);
  const handleCapture = () => { if (videoRef.current && canvasRef.current) { const video = videoRef.current; const canvas = canvasRef.current; canvas.width = video.videoWidth; canvas.height = video.videoHeight; const context = canvas.getContext('2d'); if (!context) return; context.translate(canvas.width, 0); context.scale(-1, 1); context.drawImage(video, 0, 0, canvas.width, canvas.height); const imageDataUrl = canvas.toDataURL('image/png'); setCapturedImage(imageDataUrl); setAppState('CONFIRM'); } };
  const handleRetake = () => { setCapturedImage(null); setProgress(0); setCompletedAnalyses(0); setAppState('CAMERA'); };
  const handleAnalyze = () => { setAppState('ANALYZING'); };
  ...
  if (appState === 'RESULTS') {
    const analysesList = ["Analisa bentuk wajahmu", "Analisa tone kulitmu", "Analisa kecocokan gaya selebriti", "Rekomendasi hijab personal"];
    return (
      <main className="flex flex-col items-center justify-center h-screen w-screen bg-pink-100 text-gray-800 p-4 transition-colors duration-500">
        ...
        <div className="mt-12 w-full max-w-sm flex flex-col gap-3">
          {analysesList.map((label, index) => {
            ...
            return (
              <button key={index} className={`w-full p-3 font-semibold rounded-xl flex items-center justify-between transition-all duration-500 ${isCompleted ? "bg-gray-800 text-white" : "bg-white text-gray-500 border border-gray-200"}`}>
                <span>{label}</span>
                {isCompleted ? (<div className="w-6 h-6 bg-white rounded-full flex items-center justify-center"><CheckIcon className="text-gray-800" /></div>) : (<AnalysisIcon className="stroke-gray-400"/>)}
              </button>
            )
          })}
        </div>
